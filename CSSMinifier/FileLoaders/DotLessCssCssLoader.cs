...
namespace CSSMinifier.FileLoaders
{
	...
	public class DotLessCssCssLoader : ITextFileLoader
    {
        ...
        private class MarkerIdTidyingRuleset : Ruleset
        {
			private readonly InsertedMarkerRetriever _markerIdRetriever;
			private readonly string _optionalTagNameToRemove;
			public MarkerIdTidyingRuleset(NodeList<Selector> selectors, NodeList rules, InsertedMarkerRetriever markerIdRetriever, string optionalTagNameToRemove) : base(selectors, rules)
			{
				if (markerIdRetriever == null)
					throw new ArgumentNullException("markerIdRetriever");
                ...
                foreach (var path in paths)
                {
                    ...
                    if (numberOfMarkerIds == 0)
                    {
						...
						else
						{
							...
							var segmentsWithoutOptionalTagNameToRemove = path.Where(s => string.Join("", s.Elements.Select(e => e.Value)) != _optionalTagNameToRemove);
							if (segmentsWithoutOptionalTagNameToRemove.Any())
								yield return segmentsWithoutOptionalTagNameToRemove;
						}
                        continue;
                    }
